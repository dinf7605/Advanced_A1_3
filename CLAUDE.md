...
| 응답 지연 | **클라이언트 캐시 + 경량 모델 + 체감 완화.** 프롬프트 축약은 효과 없어 미채택 | §12.5 |
...
- **지연을 줄이려고 프롬프트를 줄이지 마십시오.** 실측: 응답 시간 ≈ **출력 토큰 × 6.7ms**.
  입력을 3.3배로 늘려도 ms/출력토큰이 같았습니다 — **입력은 지연에 기여하지 않습니다.**
  지연을 줄이는 방법은 ① 캐시(호출을 없앤다) ② 경량 모델 ③ 체감 완화뿐입니다 (PRD §12.5).
  입력 축약은 **쿼터**에는 효과가 있습니다 — 목적이 다릅니다.
- **캐시를 숨기지 않는다.** 적중 시 `저장된 결과` 배지 + [새로 만들기] 버튼을 보여줍니다.
  조용히 옛 결과를 주면 사용자가 서비스가 고장 났다고 생각합니다.
...
- [x] **응답 시간 ≈ 출력 토큰 × 6.7ms** (2026-08-31). 같은 입력 3회 반복에서
      ms/출력토큰 6.58~6.93으로 일정했고, 입력을 3.3배(1,320→4,301)로 늘려도 6.29로 같았다.
      → **프롬프트 축약은 지연을 줄이지 못한다.** 캐시·모델·체감 완화가 실효 지렛대다.
