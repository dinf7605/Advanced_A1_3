...
**문서** — [서비스 기획서](docs/서비스기획서.md) · [PRD](doesaegim_PRD.md) · [AI 도구 사용 로그](docs/ai-tool-log/)
...
**대표 사용 시나리오** — 수업 후 필기를 통째로 복사 → [되새김] 탭에 붙여넣고 과목·난이도 선택
→ 10초 뒤 요약 3줄·용어·퀴즈 5문항 → 퀴즈를 풀면 즉시 채점 + 해설 → [내 기록]에 저장돼 지하철에서 다시 열어봄.

### 페이지 구성 · 핵심 기능

**단일 `index.html` + 해시 라우팅**으로 4개 섹션을 전환합니다.

| 섹션 | 주소 | 핵심 기능 | AI |
|------|------|-----------|:--:|
| 홈 | `#/` | 서비스 소개, 3단계 사용법 | — |
| **되새김** | `#/study` | 노트 입력 → 요약 3줄 · 용어 3~5개 · 퀴즈 5문항, **즉시 채점 + 해설**, Markdown 복사 | ✅ |
...
| 화면 | 동작 |
|------|------|
| 데스크톱 · 태블릿 (600px~) | 상단 가로 메뉴. 현재 섹션은 밑줄 + `aria-current="page"` |
| 모바일 (~599px) | 햄버거 버튼 → 전체 화면 오버레이. 링크를 누르면 자동으로 닫힘 |

브라우저 **뒤로 가기**로도 섹션이 전환되고(`hashchange`), 알 수 없는 주소(`#/asdf`)는
빈 화면 대신 **홈으로 되돌립니다.**

부가 기능 — 다크 모드(시스템 설정 자동 감지 + 수동 토글 기억), 스켈레톤 로딩 · 토스트 등
마이크로 인터랙션.

### AI 기능 명세 — 입력 / 출력 / 실패 처리

> 전체 기획 문서는 **[서비스 기획서](docs/서비스기획서.md)**에 있습니다. 아래는 요약입니다.

#### 입력

| 필드 | 형태 | 필수 | 규칙 |
|------|------|:----:|------|
| **노트** | 여러 줄 텍스트 | ✅ | 공백 제외 **100자 이상 4,000자 이하** |
| **과목** | 드롭다운 | — | 일반 / IT / 경영 / 과학 / 어학 · 기본값 `일반` |
| **난이도** | 라디오 | — | 쉬움 / 보통 / 어려움 · 기본값 `보통` |

- 실시간 글자 수 카운터가 있고, 4,000자를 넘으면 빨갛게 표시됩니다.
- **검증은 화면과 서버 양쪽에서** 합니다. 화면 검증은 UX, **서버 검증은 보안**입니다 —
  화면 검증만 있으면 `curl`로 우회해 대량 요청을 보낼 수 있습니다.
- **100자 하한** — 한 문장을 넣으면 요약할 것도 문제로 낼 것도 없어 모델이 내용을 지어냅니다.
- **4,000자 상한** — 넘치면 **자르지 않고 알립니다.** 조용히 자르면 뒷부분이 빠진 이유를 알 수 없습니다.
...
| **퀴즈** | 정확히 5문항 | **O/X 2 + 객관식 3.** 각각 보기·정답·해설 |

- 카드 4장(주제 / 요약 / 용어 / 퀴즈)으로 표시되고 위에서 아래로 순서대로 나타납니다.
- 퀴즈를 풀고 [채점하기] → **점수 · 정오 표시 · 해설**. 정오는 색뿐 아니라 ✅/❌ 아이콘으로도 구분합니다.
- **해설은 채점 전에는 DOM에 넣지 않습니다.**
- 과목·난이도가 결과를 실제로 바꿉니다 — 근거는 [5-8](#5-8-과목난이도는-라벨이-아니라-지시로-넘긴다).
...
| **결과 불량** | 모델이 생성을 거절 | "이 내용으로는 퀴즈를 만들기 어려워요." | — |

**세 가지 원칙**

1. **차단은 요청을 보내지 않습니다** — 빈 입력으로 눌러도 네트워크 요청이 **0건**입니다.
2. **오류 메시지는 사용자가 할 수 있는 행동을 줍니다** — "타임아웃이 발생했습니다"(✗) → "노트를 조금 줄이면"(✓)
3. **내부 사정은 알리지 않습니다** — 키가 없어도 화면에는 "일시적인 문제"만. 원인은 `/api/health`와 로그로 확인합니다.

**응답 처리 4단계** — AI 응답은 그대로 쓸 수 없습니다.
① 잘렸는지 확인(`stop_reason`) → ② 코드블록 껍데기 벗기기 → ③ JSON 파싱(실패 시 **1회만** 재시도)
→ ④ **의미 검증**(정답이 보기 안에 있는지 등). 자세한 이유는 [5-3](#5-3-json-출력을-어떻게-강제할-것인가).
